...
namespace IDosGames
{
    public class UserInventory
    {
        ...
        //Этот Метод Добавлен во время задания
        public static List<(string ItemID, int Amount)> GetAll()
        {
            return _eachItemAmounts.Select(entry => (entry.Key, entry.Value)).ToList();
        }
    }
}